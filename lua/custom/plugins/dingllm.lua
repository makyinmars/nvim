return {
  ...
  config = function()
    ...
    local function create_llm_functions(provider)
      return {
        replace = function()
          invoke_llm(provider.url, provider.model, provider.api_key, global_system_prompt, true, provider.spec.make_args, provider.spec.handle_data)
        end,
        help = function()
          invoke_llm(provider.url, provider.model, provider.api_key, helpful_prompt, false, provider.spec.make_args, provider.spec.handle_data)
        end,
      }
    end
