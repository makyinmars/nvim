return {
  ...
  config = function(self)
    ...
    vim.keymap.set({ 'n', 'v' }, '<leader>kdd', invoke_with_opts { debug = true }, { desc = 'Send current selection to LLM debug' })
    vim.keymap.set({ 'n', 'v' }, '<leader>kdb', invoke_with_opts { debug = false }, { desc = 'Send current selection to LLM llm_fill' })
